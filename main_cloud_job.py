...
import graph_generator as model_lib
...
class Evaluator(object):
  ...
  def evaluate(self, num_eval_batches=None):
    ...
    with tf.Graph().as_default() as graph:
      self.tensors = self.model.build_eval_graph(self.eval_batch_size)
  ...
  def write_predictions(self):
    ...
    with tf.Graph().as_default() as graph:
      self.tensors = self.model.build_eval_graph(self.data_dir, self.batch_size)
      self.saver = tf.train.Saver()
    self.sv = tf.train.Supervisor(
        graph=graph,
        logdir=self.output_path,
...
class Trainer(object):
  ...
  def __init__(self, args, model, cluster, task):
    ...
    self.task = task
    self.evaluator = Evaluator(self.args, self.model, self.args.data_dir,
                               'eval_set')
    self.train_evaluator = Evaluator(self.args, self.model, self.args.data_dir,
                                     'train_set')
  ...
  def run_training(self):
    ...
    if self.is_master and self.task.index > 0:
      raise AssertionError('Only one replica of master expected')
    ...
    with tf.Graph().as_default() as graph:
      with tf.device(device_fn):
        ...
        self.tensors = self.model.build_train_graph(self.args.data_dir,
        ...
        self.summary_writer = tf.summary.FileWriter(
            self.args.output_path, max_queue=500, flush_secs=5)
        ...
        with self.sv.managed_session(target, config=config) as session:
          ...
          while not self.sv.should_stop() and self.global_step < max_steps:
            try:
              ...
              # Partial Train Correct discriminator training taken from:
              # https://github.com/hardmaru/cppn-gan-vae-tensorflow/blob/master/model.py
             
              disc_result, self.global_step = session.run([
                      self.tensors.discLoss,
                      self.tensors.global_step])
              
              for _ in range(5):
                _, gen_result, self.global_step = session.run([
                    self.tensors.genOptimizer,
                    self.tensors.genLoss, self.tensors.global_step
                ])
                if gen_result < LOWER_THRESHOLD:
                  break

              if disc_result > LOWER_THRESHOLD and gen_result < UPPER_THRESHOLD:
                _, self.global_step = session.run([
                        self.tensors.discOptimizer,
                        self.tensors.global_step])
              ...
              should_write_summaries = self.is_master and (
                  self.global_step -
                  self.last_step_summary) > self.summary_interval
  ...
  def save_summaries(self, session):
    self.last_step_summary = self.global_step
